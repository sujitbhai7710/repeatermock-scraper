...
async def run_incremental_scrape(
    ...
    async def wait_for_live_account():
        ...
        while True:
            ...
            # 5. GUEST MODE — no account needed at all! A fresh guest session is
            #    generated by just visiting the site (works for all FREE 'tb'
            #    tests: questions + attempt + submit + solutions + analysis).
            #    Regenerates automatically forever — ideal for GitHub Actions.
            if os.environ.get("ALLOW_GUEST", "1") == "1":
                print("  👤 Falling back to GUEST MODE (no account — free 'tb' tests only)")
